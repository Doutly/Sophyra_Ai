            ...
            <div className="lg:col-span-8 space-y-4 sm:space-y-5">
              ...
              <div className="bg-white rounded-2xl border border-slate-100 shadow-sm">
                <div className="px-4 sm:px-6 py-4 border-b border-slate-50 flex items-center justify-between">
                  <div>
                    <p className="text-xs text-slate-400">{reports.length} total interviews</p>
                  </div>
                ...
                {mockRequests.length === 0 ? (
                  ...
                  <div className="p-3 sm:p-4 space-y-3">
                    <InterviewRequestCard
                      ...
                      scheduledTime={mockRequests[0].scheduled_time}
                      interviewUrl={`${window.location.origin}/interview/setup?ticket=${mockRequests[0].ticket_number}`}
                      onStartInterview={() => {
                        if (mockRequests[0].status === 'approved') {
                    ...
                    {mockRequests.length > 1 && (
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-xs font-semibold text-slate-600 bg-slate-50 hover:bg-slate-100 border border-slate-200 hover:border-slate-300 transition-all"
                      >
                        <Ticket className="w-3.5 h-3.5" />
                        <span>Show {mockRequests.length - 1} more {mockRequests.length - 1 === 1 ? 'request' : 'requests'}</span>
                      </button>
                    )}
                  </div>
                )}
