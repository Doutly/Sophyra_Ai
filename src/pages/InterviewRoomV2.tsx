...
export default function InterviewRoomV2() {
  ...
  return (
    <div className="h-screen bg-slate-50 flex flex-col overflow-hidden select-none relative">
      {/* 
        MAIN INTERVIEW UI 
        We keep this mounted at all times to ensure SimliAvatarPanel and user video 
        maintain a stable lifecycle, avoiding redundant connection attempts.
      */}
      <div className={`flex flex-col h-full w-full grayscale-[0.2] transition-all duration-700 ${!started || ended ? 'opacity-0 pointer-events-none scale-95 blur-sm' : 'opacity-100'}`}>
      ...
      {/* OVERLAYS */}
      {!started && !ended && (
        <div className="absolute inset-0 z-50 overflow-auto bg-slate-50">
          <PreInterviewScreen
            session={session}
            candidateName={candidateNameRef.current}
            videoRef={videoRef}
